...
# Per-cluster lock: prevents two controllers targeting the SAME SUT from issuing
# overlapping PG restarts that would abort each other's in-flight benchmark.
# Independent runs against different SUTs get different lock files and never block.
LOCKFILE="${LOCKFILE:-/tmp/run_multiple_times.sh.${SUT//[^a-zA-Z0-9]/_}.lock}"
...
# ---------------------------------------------------------------------------
# Per-cluster temp file prefix: all log/summary paths are scoped to this SUT so
# concurrent runs against different clusters never share files.
TMPDIR_PREFIX="/tmp/repro_${SUT//[^a-zA-Z0-9]/_}"

echo "======== PROVISION (once) ========"
sutssh "cd ${REPRO_PATH} && ${SUT_ENV} ./run.sh postgresql SUT --ldg=${LDG} install configure" >"${TMPDIR_PREFIX}_provision_sut.log" 2>&1 &
sleep "$SLEEP_BETWEEN"
ldgssh "cd ${REPRO_PATH} && ${LDG_ENV} ./run.sh postgresql LDG --sut=${SUT} install configure" >"${TMPDIR_PREFIX}_provision_ldg.log" 2>&1
wait 2>/dev/null
echo "provision done (SUT: ${TMPDIR_PREFIX}_provision_sut.log, LDG: ${TMPDIR_PREFIX}_provision_ldg.log)"
...
for i in $(seq 1 "$ITERS"); do
  ...
  sutssh "cd ${REPRO_PATH} && ${SUT_ENV} ./run.sh postgresql SUT --ldg=${LDG} load" >"${TMPDIR_PREFIX}_sut_load_$i.log" 2>&1 &
  sleep "$SLEEP_BETWEEN"
  ldgssh "cd ${REPRO_PATH} && ${LDG_ENV} ./run.sh postgresql LDG --sut=${SUT} load" >"${TMPDIR_PREFIX}_ldg_load_$i.log" 2>&1
  wait 2>/dev/null

  # --- restart #2 (post-load, pre-benchmark -> cold buffers) + run step ---
  [ "$RESTART_PG" = true ] && pg_restart
  sutssh "cd ${REPRO_PATH} && ${SUT_ENV} ./run.sh postgresql SUT --ldg=${LDG} run" >"${TMPDIR_PREFIX}_sut_$i.log" 2>&1 &
  sleep "$SLEEP_BETWEEN"
  ldgssh "cd ${REPRO_PATH} && ${LDG_ENV} ./run.sh postgresql LDG --sut=${SUT} run" >"${TMPDIR_PREFIX}_ldg_$i.log" 2>&1
  wait 2>/dev/null

  tps=$(grep -oP 'tps = \K[0-9.]+(?= \((?:without|excluding))' "${TMPDIR_PREFIX}_ldg_$i.log" | tail -1)
  echo "iter=$i tps=${tps:-<none>}"
  echo "iter=$i tps=${tps:-NA}" >>"${TMPDIR_PREFIX}_summary.txt"
done
...
if [ "$DO_CLEANUP" = true ]; then
  echo "======== CLEANUP ========"
  sutssh "cd ${REPRO_PATH} && ${SUT_ENV} ./run.sh postgresql SUT --ldg=${LDG} cleanup" >"${TMPDIR_PREFIX}_cleanup_sut.log" 2>&1 || true
  ldgssh "cd ${REPRO_PATH} && ${LDG_ENV} ./run.sh postgresql LDG --sut=${SUT} cleanup" >"${TMPDIR_PREFIX}_cleanup_ldg.log" 2>&1 || true
fi

echo "=== SUMMARY ==="; cat "${TMPDIR_PREFIX}_summary.txt"
